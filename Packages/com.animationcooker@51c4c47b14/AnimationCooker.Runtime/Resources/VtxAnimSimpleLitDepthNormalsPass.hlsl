...
sampler2D _PosMap, _NmlMap; // vtxanim
Varyings DepthNormalsVertex(Attributes input, uint vertexId : SV_VertexID)
{
    ...
    // begin vtxanim stuff
    float3 newPos = InterpPos(vertexId);
    input.positionOS = float4(newPos.x, newPos.y, newPos.z, 1.0);
    #if USE_VA_NORMAL_MAP
        input.normalOS = InterpNml(vertexId);
    #endif
    //#if USE_VA_TANGENT_MAP
    //    input.tangentOS = InterpTan(vertexId);
    //#endif
    // end vtxanim stuff
