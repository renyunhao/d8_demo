...
namespace AnimCooker
{
    ...
    {
        public short ForeverClipIndex; // animation goes back to this after PlayOnce finishes
        public AnimationPlayMode Mode; // current animation play mode
        public short ModelIndex; // holds the current model index
        public short CurrentClipIndex; // holds the index of the current animation that is being played
        public short LastPlayedClipIndex; // holds the clip index of the last played clip (gets set whenever a played clip finishes)
    }

    public struct AnimationSpeedData : IComponentData
    {
        public float PlaySpeed; // the current play speed
    }
